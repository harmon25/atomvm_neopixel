...
The AtomVM LedStrip library is only supported on the ESP32 platform.
...
The AtomVM LedStrip library is implemented as an AtomVM component, which includes some native C code that must be linked into the ESP32 AtomVM image.  In order to build and deploy this client code, you must build an AtomVM binary image with this component compiled and linked into the image.
...
The `atomvm_led_strip` library can be used to drive a strip of [WS2812](https://cdn-shop.adafruit.com/datasheets/WS2812.pdf) "Neopixel" LEDs or [SK6812](https://cdn-shop.adafruit.com/product-files/2757/p2757_SK6812RGBW_REV01.pdf) RGBW LEDs.
...
AtomVM programmers interface with the `atomvm_led_strip` API via the `led_strip` module, which provides operations for starting and stopping an Erlang process associated with a specified LED strip, and for setting values on each pixel in the strip.

This guide provides an overview of the interfaces provided by this module.

### Lifecyle

Use the `led_strip:start/2` function to initialize a LED strip instance.  Specify the ESP32 pin to which the LED strip is connected, as well as the number of LEDs in the strip.  This function will return a reference to a LED strip process, which is used in subsequent operations.

    %% erlang
    Pin = 18,
    NumPixels = 4,
    {ok LedStrip} = led_strip:start(Pin, NumPixels),
    ...

Use the `led_strip:start/3` function to initialize a LED strip instance with non-default options.  Options can be specified as an Erlang map or a proplist (keyword list in Elixir).  The permissible entries are encapsulated in the following table:
...
For example, to use an RGBW strip:
    ...
    Pin = 18,
    NumPixels = 4,
    {ok, LedStrip} = led_strip:start(Pin, NumPixels, #{led_type => rgbw}),
    ...

Or in Elixir with a keyword list:

    # elixir
    {:ok, led_strip} = :led_strip.start(18, 4, led_type: :rgbw)

The returned `LedStrip` instance should be used for subsequent operations.

Use the `led_strip:stop/1` function to stop a LED strip instance and free any resources in use by it.

    %% erlang
    led_strip:stop(LedStrip).

### Multiple Strips

You can drive multiple LED strips simultaneously by starting multiple LED strip instances on different GPIO pins:

    %% erlang
    %% Start three independent strips
    {ok, Strip1} = led_strip:start(18, 30),              %% 30-pixel RGB strip on pin 18
    {ok, Strip2} = led_strip:start(19, 60),              %% 60-pixel RGB strip on pin 19
    {ok, Strip3} = led_strip:start(21, 16, #{led_type => rgbw}),  %% 16-pixel RGBW strip on pin 21

Or in Elixir:

    # elixir
    {:ok, strip1} = :led_strip.start(18, 30)
    {:ok, strip2} = :led_strip.start(19, 60)
    {:ok, strip3} = :led_strip.start(21, 16, led_type: :rgbw)
...
RMT channels are automatically allocated by ESP-IDF when you call `led_strip:start`. If no channels are available, start will return an error.

> **Note:** The `channel` option in `led_strip:start/3` is legacy and ignored in ESP-IDF 5.x. Channel allocation is now automatic.

### Clearing pixels

Use the `led_strip:clear/1` function to clear all the pixels in the LED strip to an "off" value.
...
Note that setting values will not change the state of the LED strip until the LedStrip instance is refreshed (see below).

#### RGB Color Space

Pixel colors can be set using the RGB color space via the `led_strip:set_pixel_rgb/5` function.  Pixel indices are in the range `[0..NumPixels-1]`.

For example, to set the second pixel in the strip (index 1) to all red, use:

    %% erlang
    ok = led_strip:set_pixel_rgb(LedStrip, 1, 255, 0, 0).
...
For RGBW strips (SK6812), pixel colors can be set using the `led_strip:set_pixel_rgbw/6` function. This function is only available when the strip was initialized with `led_type => rgbw`.

For example, to set the second pixel to red with 50% white:

    %% erlang
    ok = led_strip:set_pixel_rgbw(LedStrip, 1, 255, 0, 0, 128).

Or in Elixir:

    # elixir
    :ok = :led_strip.set_pixel_rgbw(led_strip, 1, 255, 0, 0, 128)
...
Pixel colors can be set using the HSV color space via the `led_strip:set_pixel_hsv/5` function.  Pixel indices are in the range `[0..NumPixels-1]`.
...
For example, to set the second pixel in the strip (index 1) to all red, use:
    ...
    %% erlang
    ok = led_strip:set_pixel_hsv(LedStrip, 1, 0, 100, 100).
...
For RGBW strips, use `led_strip:set_pixel_hsvw/6` to set a pixel using HSV values plus an independent white channel. The HSV values control the RGB LEDs while the white parameter controls the dedicated white LED.

    %% erlang
    %% Set pixel 0 to orange (H=30) at full saturation and value, with 50% white
    ok = led_strip:set_pixel_hsvw(LedStrip, 0, 30, 100, 100, 128).

Or in Elixir:

    # elixir
    :ok = :led_strip.set_pixel_hsvw(led_strip, 0, 30, 100, 100, 128)
...
Use the `led_strip:fill_rgb/4` function to set all pixels in the strip to the same color with a single call.

    %% erlang
    %% Fill entire strip with red
    ok = led_strip:fill_rgb(LedStrip, 255, 0, 0).

Or in Elixir:

    # elixir
    :ok = :led_strip.fill_rgb(led_strip, 255, 0, 0)

For RGBW strips, use `led_strip:fill_rgbw/5`:

    %% erlang
    %% Fill entire strip with pure white (using white LED only)
    ok = led_strip:fill_rgbw(LedStrip, 0, 0, 0, 255).

> **Note:** Calling `fill_rgbw/5` on an RGB strip will return `{error, not_supported}`.

#### Filling with HSV Colors

Use `led_strip:fill_hsv/4` to fill the strip using the HSV color space. This is convenient for color cycling effects like rainbows:

    %% erlang
    %% Fill entire strip with red (hue=0)
    ok = led_strip:fill_hsv(LedStrip, 0, 100, 100).
    
    %% Fill with cyan (hue=180) at 50% brightness
    ok = led_strip:fill_hsv(LedStrip, 180, 100, 50).

Or in Elixir:

    # elixir
    # Rainbow cycle - just increment hue each frame
    :ok = :led_strip.fill_hsv(led_strip, hue, 100, 50)

For RGBW strips, use `led_strip:fill_hsvw/5` to combine HSV color with the white channel:

    %% erlang
    %% Warm white: orange tint (H=30) plus white LED
    ok = led_strip:fill_hsvw(LedStrip, 30, 50, 50, 200).

> **Note:** Calling `fill_hsvw/5` on an RGB strip will return `{error, not_supported}`.

### Setting Multiple Pixels

Use `led_strip:set_pixels_rgb/2` to set multiple pixels at once from a list of `{R, G, B}` tuples. Pixels are set starting at index 0.
...
Or in Elixir:
    ...
    # elixir
    colors = [{255, 0, 0}, {0, 255, 0}, {0, 0, 255}]
    :ok = :led_strip.set_pixels_rgb(led_strip, colors)

Use `led_strip:set_pixels_rgb/3` to set pixels starting at a specific offset:

    %% erlang
    %% Set pixels 5, 6, 7 to red, green, blue
    Colors = [{255, 0, 0}, {0, 255, 0}, {0, 0, 255}],
    ok = led_strip:set_pixels_rgb(LedStrip, 5, Colors).

Or in Elixir:

    # elixir
    :ok = :led_strip.set_pixels_rgb(led_strip, 5, colors)

For RGBW strips, use `led_strip:set_pixels_rgbw/2` or `led_strip:set_pixels_rgbw/3` with `{R, G, B, W}` tuples:

    %% erlang
    %% Set pixels 2, 3, 4 with different white levels
    Colors = [{255, 0, 0, 0}, {0, 255, 0, 128}, {0, 0, 255, 255}],
    ok = led_strip:set_pixels_rgbw(LedStrip, 2, Colors).
...
Use the `led_strip:refresh/1` function to ref refresh all the pixels in the LED strip.
...
Use the `led_strip:set_brightness/2` function to set the global brightness for the strip. Brightness is a value from 0 (off) to 255 (full brightness).
...
Or in Elixir:
    ...
    # elixir
    :ok = :led_strip.set_brightness(led_strip, 128)
...
Use the `led_strip:get_brightness/1` function to get the current brightness:
...
The LED strip driver is implemented as a `gen_server` process, which has important implications for concurrent access.

#### Safety

**Yes, you can safely call led_strip functions from multiple processes.** All API functions (`set_pixel_*`, `fill_*`, `set_pixels_*`, `refresh`, etc.) use synchronous `gen_server:call/2`, which means:
...
Example with multiple processes:
    ...
    %% erlang
    {ok, LedStrip} = led_strip:start(18, 30),
    
    %% Process 1: animate first half of strip
    spawn(fun() -> animate_section(LedStrip, 0, 14) end),
    
    %% Process 2: animate second half of strip
    spawn(fun() -> animate_section(LedStrip, 15, 29) end),
    
    %% Process 3: periodically refresh
    spawn(fun() -> refresh_loop(LedStrip, 16) end).  %% 60 FPS
...
1. **Batch updates**: Use `set_pixels_rgb/2,3` instead of multiple `set_pixel_rgb/5` calls - one NIF call vs N calls:
        ...
        [led_strip:set_pixel_rgb(NP, I, R, G, B) || I <- lists:seq(0, 29)].
        
        %% Fast: 1 gen_server call  
        led_strip:set_pixels_rgb(NP, Colors).
...
For custom timeout handling:
    ...
    %% erlang
    try
        led_strip:set_pixel_rgb(LedStrip, 0, 255, 0, 0)
    catch
...
from the top level of the `atomvm_led_strip` source tree.  Output is written to the `doc` directory.
...
```erlang
%% Current (tuple list)
led_strip:set_pixels_rgb(Strip, [{255,0,0}, {0,255,0}, {0,0,255}]).

%% Potential future API (binary)
led_strip:set_pixels_rgb_bin(Strip, <<255,0,0, 0,255,0, 0,0,255>>).
```
...
```erlang
%% Current (blocking)
ok = led_strip:refresh(Strip).

%% Potential future API (non-blocking)  
ok = led_strip:refresh_async(Strip).
```
